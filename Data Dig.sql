...
/*Display worldwide average, max, and min values of profit*/
SELECT AVG(worldwide) * 1000000 AS "Top 100 Movies: Average Worldwide Profit (USD)" FROM topmovies;
SELECT MAX(worldwide) * 1000000 AS "Top 100 Movies: Max Worldwide Profit (USD)",title,year FROM topmovies;
SELECT MIN(worldwide) * 1000000 AS "Top 100 Movies: Min Worldwide Profit (USD)",title,year FROM topmovies;

/*Display worldwide average, max, and min profit from movies that are from after 2012*/

SELECT AVG(worldwide) * 1000000 AS "Top 100 Movies After 2012: Average Worldwide Profit (USD)" FROM
(SELECT worldwide FROM topmovies GROUP BY year HAVING year > 2012);

SELECT MAX(worldwide) * 1000000 AS "Top 100 Movies After 2012: Max Worldwide Profit (USD)" FROM (SELECT worldwide FROM topmovies GROUP BY year HAVING year > 2012);

SELECT MIN(worldwide) * 1000000 AS "Top 100 Movies After 2012: Min Worldwide Profit (USD)" FROM (SELECT worldwide FROM topmovies GROUP BY year HAVING year > 2012);


/*Display ways to group data values that don't exist yet (CASE)*/
SELECT title as "Movie Title", 
CASE
    WHEN worldwide > 1000 THEN "Billion Dollar Hit!"
    WHEN worldwide < 1000 AND worldwide > 500 THEN "More Than Halfway To a Billion Dollars"
    ELSE "Less than Half a Billion"
END AS "Is it a Billion-Dollar Movie?" 
FROM topmovies;

/*Display Top 100 Movies that made over 1B and are made after 2012(AND)*/

SELECT title AS "Movie Title" ,year, worldwide * 1000000 AS "Worldwide Profit (USD)" FROM topmovies WHERE worldwide > 1000 AND year > 2012;
/*Display Top 100 Movies from 2012 Onwards or Before 2002 (OR)*/
SELECT title AS "Movie Title", year FROM topmovies WHERE year > 2012 OR year < 2002;